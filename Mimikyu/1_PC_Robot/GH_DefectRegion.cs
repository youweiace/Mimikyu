...
namespace Mimikyu.Polyga
{
    public class GH_ScanRegion : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetDataList(4, scanObjects))
                return;
            ...
            foreach (string imageName in allDefects.Keys)
            {
                ...
                for (int i = 0; i < contours.Count; i++)
                {
                    ...
                    //                                        targetPlane
                    //                                    );
                    for (int objIndex = 0; objIndex < scanObjects.Count; objIndex++)
                    {
                        List<PixelObjectHit> objectHits =
                            ProjectPixelsToObjectMesh(
                                ...
                                poseIndex,
                                contours[i],
                                scanObjects[objIndex].Mesh,
                                scanObjects[objIndex].ObjectPlane);
                        ...
                        if (pts.Count != 0)
                        {
                            for (int p = 0; p < pts.Count; p++)
                            {
                                ...
                                {
                                    case "posX":
                                        GH_Path pathPosX = new GH_Path(objIndex, poseIndex, (int)Sides.posX, i);
                                        pixelHits.Add(pts[p], pathPosX);
                                        break;
                                    case "posY":
                                        GH_Path pathPosY = new GH_Path(objIndex, poseIndex, (int)Sides.posY, i);
                                        pixelHits.Add(pts[p], pathPosY);
                                        break;
                                    case "negX":
                                        GH_Path pathNegX = new GH_Path(objIndex, poseIndex, (int)Sides.negX, i);
                                        pixelHits.Add(pts[p], pathNegX);
                                        break;
                                    case "negY":
                                        GH_Path pathNegY = new GH_Path(objIndex, poseIndex, (int)Sides.negY, i);
                                        pixelHits.Add(pts[p], pathNegY);
                                        break;
                                    case "posZ":
                                        GH_Path pathPosZ = new GH_Path(objIndex, poseIndex, (int)Sides.posZ, i);
                                        pixelHits.Add(pts[p], pathPosZ);
                                        break;
                                    case "negZ":
                                        GH_Path pathNegZ = new GH_Path(objIndex, poseIndex, (int)Sides.negZ, i);
